...
class Input(object):
    ...
    def __init__(self, raw_request=None,
        ...
        self.port = port
        self.method = method
        #用于增加请求的页面，如192.168.88.244/login.php.则写成self.uri = "/login.php"+uri
        self.uri = "/"+uri
        ...
        if self.data != '':
            ...
            if 'Content-Type' in headers.keys():
                if headers['Content-Type'] == 'application/x-www-form-urlencoded' and stop_magic is False:
                    #修改2：urllib.unquote为python3的urllib.parse.unquote
                    if urllib.parse.unquote(self.data) == self.data:
                        #修改3：urlparse.unquote为python3的urllib.parse.parse_qsl
                        query_string = urllib.parse.parse_qsl(self.data)
                        if len(query_string) != 0:
                            #修改4：urllib.urlencode为python3的urllib.parse.urlencode
                            encoded_args = urllib.parse.urlencode(query_string)
