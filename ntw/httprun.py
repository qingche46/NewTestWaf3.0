...
class HttpResponse(object):
    ...
    def parse_content_encoding(self, response_headers, response_data):
        ...
        response_data
        解析包含内容编码的响应，以检索response_data
        """
        if response_headers['content-encoding'] == 'gzip':
            #修改7:StringIO.StringIO修改为python3的io.StringIO
            ...
            data = io.StringIO(zlib.decompress(response_data))
    ...
    def check_for_cookie(self, cookie):
        ...
        # Check if our originDomain is an IP 检查我们的原始域名是否是IP
    ...
    def send_request(self, http_request):
        ...
        try:
            ...
            raise errors.TestError(
                ...
                {
                    'msg': err,
                    'function': 'httprun.HttpUA.send_request'
                })
        finally:
            self.get_response()
    ...
    def build_request(self):
        ...
        if self.request_object.headers != {}:
            for hname, hvalue in self.request_object.headers.items():
                #修改6：unicode修改为python3的str，包含多出替换
                headers += str(hname) + ': ' + str(hvalue) + self.CRLF
        request = str.replace(request, '#headers#', headers)
        ...
        if self.request_object.data != '':
            ...
            request = str.replace(request, '#data#', data)
        else:
            request = str.replace(request, '#data#', '')
